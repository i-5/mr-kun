 ...
 * @version $Id: BaseServlet.java,v 1.1.1.1.2.24 2001/11/13 07:56:55 rick Exp $
 */
public abstract class BaseServlet extends HttpServlet
{
	...
	*/
    ...
    String wlCookie = getWLCookie(pReq);
    ...
    catch (NumberFormatException errNF)
    {
	  ...
	  try
    {
      ThreadLogger.log("\"" + wlCookie + "\",\"" + subServletName + "\",\"doAction\",\"Start\"");
	    this.doAction(pReq, pRes);
      ThreadLogger.log("\"" + wlCookie + "\",\"" + subServletName + "\",\"doAction\",\"End\"");
	  }
	...
	*/
	...
	public void forwardToTemplate(String pTemplate, HttpServletRequest pReq, HttpServletResponse pRes)
	{
	  ...
	  try
      {
		...
		context.getRequestDispatcher(pTemplate).include(pReq, pRes);
	  } 
...
*/
  ...
  public static String getWLCookie(HttpServletRequest req)
  {
    Cookie cookies[] = req.getCookies();
    String wlCookie = "no cookie";
    for (int n = 0; n < cookies.length; n++)
      if (cookies[n].getName().equals("WebLogicSession"))
      {
        wlCookie = cookies[n].getValue();
        break;
      }
    return wlCookie;
  }

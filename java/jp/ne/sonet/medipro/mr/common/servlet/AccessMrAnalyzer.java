 ...
 * ユーザ環境取得・解析用モジュール.
 */
public class AccessMrAnalyzer {
    private String mrid = null;

    public AccessMrAnalyzer(String mrid) {
	this.mrid = mrid;
    }

    public void analyze(HttpServletRequest req) {

	AccessMrLogInfo info = new AccessMrLogInfo();
	info.setMrId(mrid);
	...
	try {
	    ...
	    try {
		AccessMrLogInfoManager manager = new AccessMrLogInfoManager(conn);
		manager.addLog(info);
		conn.commit();

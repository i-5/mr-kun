 ...
 * @author: 
 */
public class DrSendMessageController {
     ...
     */
    public DrSendMessageController() {
	...
	try {
	    ...
	    msginfo = msgmanager.getDrSendMessage(messageHeaderID);
	} catch (MrException e) {
	...
	try {
	    MsgManager msgmanager = new MsgManager(conn);
	    msgheaderid = msgmanager.getDrLastSendMsg(drID, messageHeaderID, true);
	} catch (MrException e) {
	    DispatManager dispat= new DispatManager();
	    dispat.distribute(req, res);
	} finally {
...
//System.out.println("前へ新しい="+msgheaderid);
	...
	try {
	    MsgManager msgmanager = new MsgManager(conn);
	    msgheaderid = msgmanager.getDrNextSendMsg(drID, messageHeaderID, true );
	} catch (MrException e) {
	    DispatManager dispat= new DispatManager();
	    dispat.distribute(req, res);
	} finally {

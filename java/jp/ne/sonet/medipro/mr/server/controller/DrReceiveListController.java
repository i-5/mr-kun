 ...
 * @author: 
 */
public class DrReceiveListController {
     ...
     */
    public DrReceiveListController() {
	...
	try {
	    ...
	    enum = msgmanager.getDrRecvMsgList(drID,
					       sortKey,
					       rowType,
	...
	try {
	    MsgManager msgmanager = new MsgManager(conn);
	    msgmanager.updateDrRecvMsgStatus(messageHeaderID, SysCnst.MSG_STATUS_DUST); 
	} catch (MrException e) {
	    DispatManager dispat= new DispatManager();
	    dispat.distribute(req, res);
	} finally {
	...
	try {
		MsgManager msgmanager = new MsgManager(conn);
		msgmanager.updateDrRecvMsgStatus(messageHeaderID, SysCnst.MSG_STATUS_SAVE); 
	} catch (MrException e) {
		DispatManager dispat= new DispatManager();
		dispat.distribute(req, res);
	} finally {

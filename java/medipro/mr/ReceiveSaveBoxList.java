		...
		try {
			...
			if ( sm.check(request, 1) ) {
				...
				if (request.getParameter("init") != null) {
					Vector list = (Vector)session.getValue("ReceiveSaveBoxList_headerIdList");
					if (list != null) {
						Enumeration enum = list.elements();
						while (enum.hasMoreElements()) {
							session.removeValue((String)enum.nextElement());
						}
						session.removeValue("ReceiveSaveBoxList_headerIdList");
					}
				}		
...
//  				}
				...
				// reset of the session object
				sm.reset(request,0);  

				// 削除処理
				session.putValue("ReceiveSaveBoxList_moveconf"," ");
				session.putValue("ReceiveSaveBoxList_moveaction"," ");
				session.putValue("ReceiveSaveBoxList_removeconf"," ");
				session.putValue("ReceiveSaveBoxList_removeaction"," ");
				session.putValue("ReceiveSaveBoxList_msg"," ");
   

				if ( request.getParameter("ReceiveSaveBoxList_submitname") != null ) {
					// 「ごみ箱へ」のイメージを押したとき
					if ( request.getParameter("ReceiveSaveBoxList_submitname").equals("RemovePre") ) {
						if ( inputCheck(request) ) {
							session.putValue("ReceiveSaveBoxList_removeconf","ON");
							getCheckBox(request,response,session);
						}
						else {
							session.putValue("ReceiveSaveBoxList_removeconf","OFF");
						}
					}
					// 「ごみ箱のOK/Cancel」のボタンを出すとき
					else if ( request.getParameter("ReceiveSaveBoxList_submitname").equals("Remove") ) {
						// 「ごみ箱のOK」のボタンを押されたとき
						if ( request.getParameter("ReceiveSaveBoxList_removeaction").equals("OK") ) {
							session.putValue("ReceiveSaveBoxList_msg","removeOK");
							getCheckBox(request,response,session);
								// リムーブする
							msgRemove(request,response,session);
						}
						// 「ごみ箱のCancel」のボタンを押されたとき
						else if ( request.getParameter("ReceiveSaveBoxList_removeaction").equals("Cancel") ) {
							session.putValue("ReceiveSaveBoxList_msg","removeCancel");
							getCheckBox(request,response,session);
						}
				...
				// Go to the next page
				response.sendRedirect("/medipro/mr/MrReceiveSaveBoxList/index.html");
			}
			else {
				// セッションエラーの場合
    ...
    void getCheckBox(HttpServletRequest req,
		...
		session.putValue("ReceiveSaveBoxList_headerIdList", list);
    }
    ...
    Enumeration getMsgHeadId(HttpServletRequest req, HttpSession session) {
		...
		session.putValue("ReceiveSaveBoxList_headerIdList", msgid);
